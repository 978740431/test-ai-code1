            ...
            <thead className="bg-slate-50/80 dark:bg-gray-800/80 text-[#5c738a] dark:text-gray-400 border-b border-border-slate dark:border-gray-700 backdrop-blur-sm sticky top-0 z-20">
              <tr>
                <th className="w-16 py-5 text-center">
                  <input 
                    type="checkbox" 
                    className="rounded-md h-5 w-5 text-primary focus:ring-primary border-slate-300 dark:bg-gray-700 cursor-pointer"
                    onChange={(e) => {
                ...
                <th className="font-black uppercase tracking-widest text-[10px] py-5">Invoice ID</th>
                    ...
                    <div className="relative inline-block text-left">
                      ...
                      {activeMenuId === inv.id && (
                        <div ref={menuRef} className="absolute right-0 mt-1 w-48 bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl shadow-2xl z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                          <div className="py-1">
                            <button onClick={() => { onRecordPayment(inv.id); setActiveMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-gray-200 hover:bg-slate-50 transition-colors"><span className="material-symbols-outlined !text-lg text-primary">payments</span>Record payment</button>
                            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-gray-200 hover:bg-slate-50 transition-colors"><span className="material-symbols-outlined !text-lg">mail</span>Email</button>
                            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-gray-200 hover:bg-slate-50 transition-colors"><span className="material-symbols-outlined !text-lg">print</span>Print</button>
                            <div className="h-px bg-slate-100 dark:bg-gray-700 my-1"></div>
                            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-red-600 dark:text-red-400 hover:bg-red-50 transition-colors"><span className="material-symbols-outlined !text-lg">delete</span>Delete</button>
                          </div>
                        </div>
                      )}
                    </div>
